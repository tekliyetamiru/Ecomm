...
import razorpay
from .models import Cart, OrderPlace, Payment, Product, Wishlist
...
@method_decorator(login_required,name="dispatch")
class CustomerRegisterionView(View):
    ...
    def post(self,request):      
        form=CustomerRegistrationForm(request.POST) # type: ignore
        if form.is_valid():          
            form.save()
...
def search(request):
    ...
    if request.user.is_authenticated:
        totalitem = len(Cart.objects.filter(user=request.user))
        wishitem = len(Wishlist.objects.filter(user=request.user))
    product = Product.objects.filter(Q(description__icontains=query))
